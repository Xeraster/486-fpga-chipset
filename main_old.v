...
module waitStateControl(
    ...
    begin
        if (lcdcounter < 15 & lcd_active)
            ...
            RDY <= 1;
        else if (cfcardcounter < 6 ~cfcard)
...
module top(
    ...
    wire WAIT_READY;
    waitStateControl wsc(lcd_active, rom_en, kbd_en, CPU_CLK, ADS, SLOT_RDY, v9958_en, cfcard_en, sram_active, lcd_en, WAIT_READY, sram_en);
    always@(*) begin
        RDY <= ~(~WAIT_READY & ADS);
    end
